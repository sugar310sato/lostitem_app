...
class MainWindow(QMainWindow):
    ...
    def init_yolo_model(self):
        """YOLOモデルの初期化（バックグラウンドで）"""
        # バックグラウンドでYOLOモデルを初期化
        self.yolo_thread = YOLOInitThread()
        self.yolo_thread.finished.connect(self.on_yolo_ready)
        self.yolo_thread.start()
    
    def on_yolo_ready(self, predictor):
        """YOLOモデルの初期化完了時の処理"""
        self.yolo_predictor = predictor
        print("YOLOモデルの初期化が完了しました")
